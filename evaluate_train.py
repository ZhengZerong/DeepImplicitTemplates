...
def evaluate_one_instance(dataset, class_name, instance_name, experiment_directory, checkpoint, data_dir):
    ...
    # )
    reconstructed_mesh_filename = os.path.join(
    ...
    chamfer_dist = deep_sdf.metrics.chamfer.compute_trimesh_chamfer_wo_normalization(
        ground_truth_points,
        reconstruction,
        # normalization_params["offset"],
        # normalization_params["scale"],
...
def evaluate(experiment_directory, checkpoint, data_dir, split_filename):
    ...
    #
    # if not os.path.isfile(reconstructed_mesh_filename):
    #     print('Skipping %s as it doesn\'t exists' % reconstructed_mesh_filename)
    #     continue
    #
    # ground_truth_samples_filename = os.path.join(
    #     data_dir,
    ...
    with open(
        os.path.join(
            ws.get_evaluation_dir(experiment_directory, checkpoint, True), "chamfer_train.csv"
        ),
        "w",
    ) as f:
