...
def evaluate_one_instance(dataset, class_name, instance_name, experiment_directory, checkpoint, data_dir):
    ...
    logging.debug("emd distance: " + str(emd_dist))

    return os.path.join(dataset, class_name, instance_name), emd_dist
...
def evaluate(experiment_directory, checkpoint, data_dir, split_filename):
    ...
    emd_results = p.map(evaluate_one_instance, ds, cn, inn, exd, ckp, dtd)
    print(np.mean([q[1] for q in emd_results]), np.median([q[1] for q in emd_results]))


    with open(
        os.path.join(
            ws.get_evaluation_dir(experiment_directory, checkpoint, True), "emd.csv"
        ),
        "w",
    ) as f:
        f.write("shape, emd_dist\n")
        for result in emd_results:
